class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        for i in range(1, n):
            ...
            if h >= hL:
                totalArea += lowerDiff
                print("new heighest height, increase area by", lowerDiff)
                lowerDiff = 0
                lowerDiffBasin = 0
                hL = h
                hR = h
                ...
                #if h > hR:
                #if hR == hL:
                if h > hR and hR != hL: # increase happened
                    print("h", i)
                    basin = True
                    if h > hRMax and basin:
                        hRMax = h
                        p2 = i
                        lowerDiffBasin = lowerDiff
                hR = h
                #p2 = i
                #basin = True

        if basin:
            subtractArea = (p2-p1)*(hL-hRMax)
            finalAreaToAdd = lowerDiffBasin - subtractArea
            totalArea += finalAreaToAdd
            print("final basin area increase:", finalAreaToAdd, lowerDiffBasin, subtractArea)
            print(p1, p2, hL, hRMax)
